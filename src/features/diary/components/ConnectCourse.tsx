...
export const ConnectCourse = () => {
  ...
  return (
    <div className="h-full rounded-2xl p-2 md:p-4 pb-6 w-full max-w-[800px] flex flex-col gap-3 md:gap-4 bg-white">
      <h1 className="text-xl md:text-2xl">코스 연결</h1>
      <div className="flex flex-col gap-2">
        <div className="text-sm md:text-base">연관 코스 코드</div>
        <div className="flex gap-2">
          <input 
            ...
            className="w-full h-[38px] md:h-[42px] rounded-md text-[#93000A] p-2 md:p-3 text-sm md:text-base border border-gray-300 focus:outline-none focus:ring-2 focus:ring-[#93000A]/20 focus:border-transparent" 
            placeholder="코스 ID 또는 제목으로 검색"
          />
          <button 
            onClick={() => handleInputChange({ target: { value: '' } } as React.ChangeEvent<HTMLInputElement>)}
            className="w-[90px] md:w-[120px] h-[38px] md:h-[42px] rounded-md p-2 md:p-3 text-sm md:text-base bg-third/60 cursor-pointer border border-third/60 text-white flex items-center justify-center gap-2 hover:bg-third/80 transition-all duration-200"
          >
            <FontAwesomeIcon icon={faMagnifyingGlass} className="w-[10px] h-[10px] md:w-[12px] md:h-[12px]" />
            검색
          </button>
        </div>
        <div className="mt-2 md:mt-3 border border-gray-200 rounded-lg bg-white shadow-sm max-h-32 md:max-h-40 overflow-y-auto">
          {isLoading ? (
            <div className="flex justify-center items-center h-full w-full py-3 md:py-4">
              <Spinner />
            </div>
          ) : error ? (
            <div className="text-center py-2 text-xs md:text-sm text-red-500">에러가 발생했습니다: {error.message}</div>
          ) : (
            searchResults.map((item: Course) => (
            <div 
              key={item.title} 
              onClick={() => handleCourseSelect(item.title)}
              className="p-2 md:p-3 hover:bg-third/5 text-[#93000A] cursor-pointer border-b border-gray-100 last:border-b-0"
            >
